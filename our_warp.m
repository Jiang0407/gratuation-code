function [Hg,re_Hmdlt,re_invHmdlt,InmeshX,InmeshY,in_name1,in_name2]=...
    our_warp(in_name1,in_name2,num,num_img)
% function warp_img=our_warp(in_name1,in_name2,num,num_img)
%-------------------------------------------------------------------------
...
%判定
%---------------
img1=imread(in_name1);
img2=imread(in_name2);
if num==1
    ...
    n_img1=size(img1,2);
    vo_sift=floor(n_img1*(num-1)/num); %对于大图像取一半进行配准
    [ kp1,ds1 ] = vl_sift( single( rgb2gray(img1(:,vo_sift:end,:) ) ),'PeakThresh', 0,'edgethresh',500);

%%%%%
%     figure;imshow(img1(:,vo_sift:end,:));hold on;
%     plot(kp1(1,:),kp1(2,:),'ro');
%%%%%中间两行为验证
...
if size(img1,1) < size(img2,1)
    img11=[img1;255*ones(size(img2,1)-size(img1,1),size(img1,2),3)];
    img21=img2;
elseif size(img1,1) >= size(img2,1)
    img21=[img2;255*ones(size(img1,1)-size(img2,1),size(img2,2),3)];
    img11=img1;
end
    % Show input images.
...
%使用RANSAC
%--------------------------
% RE_COO(:,:,1)=data_norm(1:2,:);
% RE_COO(:,:,2)=data_norm(4:5,:);
% [~,inliers]=RANSAC(RE_COO);
%--------------------------

if size(img11,1) == size(img21,1)
    % Show results of RANSAC.
    fprintf('  Showing results of RANSAC...');tic;
    figure;
%     if num==3
%         imshow([img1(:,vo_sift:end,:) img2]);
%     else
        imshow([img11,img21]);
%     end
    hold on;
...
%         else
%             plot(data_orig(4,inliers(i))+(size(img1,2)-vo_sift+1),data_orig(5,inliers(i)),'go','LineWidth',2);
%             plot([data_orig(1,inliers(i)) data_orig(4,inliers(i))+size(img1,2)-vo_sift+1],[data_orig(2,inliers(i)) data_orig(5,inliers(i))],'g-');
%         end
...
[InmeshX{1},InmeshY{1}]=meshgrid( linspace(1,size(img1,2),C1),linspace(1,size(img1,1),C2) );
% InmeshX{1}=InmeshX{1}(:);
% InmeshY{1}=InmeshY{1}(:);
[InmeshX{2},InmeshY{2}]=meshgrid( linspace(1,size(img2,2),C1),linspace(1,size(img2,1),C2) );
% InmeshX{2}=InmeshX{2}(:);
% InmeshY{2}=InmeshY{2}(:);
fprintf('done (%fs)\n',toc);
...
%在主函数中调用
%-------------
% warp_img=run_example_mine(Hg,re_Hmdlt,re_invHmdlt,InmeshX,InmeshY,in_name1,in_name2);
