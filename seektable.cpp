...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	double *ble_mask;
    int nStringLen;
    string szImageName1,szImageName2;
	m = (int)mxGetM(prhs[0]);
	n = (int)mxGetN(prhs[0])/3;
	sti_mask = mxGetPr(prhs[0]);
	ble_mask = mxGetPr(prhs[1]);
    
    nStringLen = mxGetNumberOfElements(prhs[2]);    
    szImageName1.resize( nStringLen + 1 );    
    mxGetString( prhs[2], &szImageName1[0], nStringLen + 1 );
    nStringLen = mxGetNumberOfElements(prhs[3]);
    szImageName2.resize( nStringLen + 1 );
    mxGetString( prhs[3], &szImageName2[0], nStringLen + 1 );
    
    
	//opencv读取视频
	IplImage *inframe1;
	IplImage *inframe2;
	inframe1 = cvLoadImage(szImageName1.c_str());
	inframe2 = cvLoadImage(szImageName2.c_str());
    cvShowImage("inframe1", inframe1);
    cvWaitKey();
