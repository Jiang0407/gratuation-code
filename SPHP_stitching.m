function [c1all,center00]=SPHP_stitching(in_name, edge_list, ref, tar, warp_type, zeroR_ON,center,num)
...
if num==1
    ...
    nor_coor=T{1}*[center(1);center(2);1];
    [cu(1), ~] = apply_transform(nor_coor(1), nor_coor(2), [cos(theta), sin(theta), 0; ...
                                         -sin(theta), cos(theta), 0; ...
                                                   0,          0, 1]); 
    [tmpx, tmpy] = apply_transform(0, 0, H{ref, i}); % transform center (0, 0) of ith img to ref img
...
oriub1 = min(cu);
oriub2 = max(cu);
img_coor=T{1}\[tmpx;tmpy;1];
center00=img_coor(1:2);
% ub1 = oriub1; ub2 = oriub2;


s_itv = 20; % sample interval
[offset_table1, offset_table2] = meshgrid(-300:10:200, -300:10:200);
...
for oj = 1 : size(offset_table1, 2)
    ...
    for i = 1 : img_n
        ...
        [x, y] = meshgrid(x, y);
%         [x, y] = apply_transform(x, y, T{i}); % coord change 3.6改
        ...
        for reg = 1 : 3 % for each region
            ...
            elseif reg == 3 % region 3
                A = c1para.S * H{ref, i}; %图4原理，ith-->tar
            end
            A = A / A(3, 3);
...
c1para = compute_c1_warp_coeff(H0, t, c, theta, ub1, ub2, zeroR_ON); %ref-->tar

%% c1 warp for each image
img_grid_size = 20; % for warping
